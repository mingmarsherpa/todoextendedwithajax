...
- Authenticated users can:
  ...
  - set or clear an in-app reminder time for a todo
...
- Todo reminders are page-scoped in-app popups:
  - a reminder popup appears while the signed-in user has the `/Todos` page open
  - reminders are scheduled client-side from reminder data rendered on the page
  - when a reminder popup fires, the todo is marked so the same reminder is not shown repeatedly
  - completed todos are excluded from pending reminder popups unless they are later marked active again with an untriggered reminder
...
- Application startup runs EF Core migrations and identity seeding before request handling.
- Database initialization retries when SQL Server is reachable but not yet ready to open the configured database.
...
- Todo reminders do not run in the background and do not currently use browser push, email, SMS, or OS-level notifications.
